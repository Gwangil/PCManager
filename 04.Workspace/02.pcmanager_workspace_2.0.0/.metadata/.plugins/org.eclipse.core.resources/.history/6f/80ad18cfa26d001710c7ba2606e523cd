...
public class FrontController extends HttpServlet {
	...
	protected void useListAll(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if (authenticationCheck(request, response)) {
			if (authorizationCheck(request, response)) {
				ArrayList<Seat> list = sDao.selectList();
	...
	protected void logout(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (authenticationCheck(request, response)) {
			if (authorizationCheck(request, response)) {
				...
				int seatNo= sDao.selectOne(memberId).getSeatNo();
				int Mileage=sDao.remainTime(seatNo);
				int tmp1 = dao.updateMileage(memberId, Mileage);
				int tmp2 = sDao.updateCondition(seatNo);
				int tmp3 = sDao.Update(seatNo);
				...
				request.setAttribute("message", "다시로그아웃하세요.");
	...
	protected void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String tmp = sDao.selectOne(seatNo).getMemberId();

		if (memberId.length() > 0 && memberPw.length() > 0) {
			String grade = dao.loginCheck(memberId, memberPw);
			int mileage = dao.selectOne(memberId).getMileage();
			if (grade != null) {
				if (grade.equals("A")) {
					...
					request.getRequestDispatcher("result.jsp").forward(request, response);
						...
						HttpSession session = request.getSession(true);
						sDao.Update( seatNo, memberId, 1, mileage);
						session.setAttribute("grade", grade);
						session.setAttribute("memberId", memberId);
						request.getRequestDispatcher("result.jsp").forward(request, response);
				} else {
					request.setAttribute("message", "결제를하고오세요^^");
					request.getRequestDispatcher("error.jsp").forward(request, response);
				}
			} else {
