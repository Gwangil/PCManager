...
<div class="menu_area" style="text-align:right;">
	...
	<c:when test="${not empty sessionScope.memberId and 
		...
		<c:if test="${sessionScope.grade == 'A'}">
			[관리자]
			<a href="pcontroller?action=getProductAll">전체회원조회</a>
			<a href="controller?action=noticeInput">공지관리</a>
			<a href="/pcm/pcontroller?action=getProductAll">상품관리</a>&nbsp;
			<a href="/pcm/pcontroller?action=getOrderAll">주문관리</a>
			<a href="/pcm/pcontroller?action=getSale">매출관리</a>
		</c:if>
	</c:when>
